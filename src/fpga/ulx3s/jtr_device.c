...
char *jtr_device_id(struct jtr_device *dev)
{
	...
	if (!dev)
		return "";
	sprintf(device_id, "%s #%d", dev->device->ulx3s_device->snString,
		dev->fpga_num + 1);
	return device_id;
}
...
void jtr_device_list_print_count()
{
	...
	fprintf(stderr, "%d device(s) ULX3S 1.15y ready\n", device_count);
}
...
int jtr_device_list_check()
{
	...
	if (found_devices_num) {
		//fprintf(stderr, "Found %d device(s) ULX3S 1.15y\n", found_devices_num);
		//ulx3s_dev_list_print(device_list_1->ulx3s_dev_list);
		...
		free(device_list_1->ulx3s_dev_list);
		free(device_list_1);
	}
...
int jtr_device_list_rw(struct task_list *task_list)
{
	...
	for (device = device_list->device; device; device = device->next) {
		...
		fprintf(stderr, "SN %s error %d doing r/w of FPGAs (%s)\n",
			device->ulx3s_device->snString, result, libusb_error_name(result) );
		...
		char *error_msg)
{
	...
	if (error_msg)
		fprintf(stderr, "SN %s: %s\n",
				device->ulx3s_device->snString, error_msg);
