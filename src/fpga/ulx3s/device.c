		...
		struct device_bitstream *bitstream)
{
	...
	if (bitstream->num_progclk) {
		...
		snprintf(conf_name_board_freq, sizeof(conf_name_board_freq),
				"Frequency_%s", device->ulx3s_device->snString);
	...
	snprintf(conf_name_board_config1, sizeof(conf_name_board_config1),
			"Config1_%s", device->ulx3s_device->snString);
...
{
	struct ulx3s_dev_list *ulx3s_dev_list_1 = ulx3s_dev_list_new();
	ulx3s_timely_scan(ulx3s_dev_list_1, device_list->ulx3s_dev_list,
		ulx3s_use_list);

	struct device_list *device_list_1 = device_list_new(ulx3s_dev_list_1);
...
struct device_list *device_init_scan(struct device_bitstream *bitstream)
{
	...
	ulx3s_init_scan(ulx3s_dev_list, ulx3s_use_list);

	struct device_list *device_list = device_list_new(ulx3s_dev_list);
...
void device_list_print(struct device_list *device_list)
{
	...
	for (dev = device_list->device; dev; dev = dev->next) {
		...
		printf("ULX3S %s bus:%d dev:%d",
				dev->ulx3s_device->snString, dev->ulx3s_device->busnum,
				dev->ulx3s_device->devnum);
	...
	for (num = 0; num < device->num_of_fpgas; num++) {
		...
		if (result < 0) {
			fprintf(stderr, "SN %s FPGA #%d fpga_select_setup_io() error: %d\n",
				device->ulx3s_device->snString, num + 1, result);
			return result;
		}
		...
		if (fpga->wr.io_state.pkt_comm_status) {
			fprintf(stderr, "SN %s FPGA #%d error: pkt_comm_status=0x%02x,"
				" debug=0x%04x\n", device->ulx3s_device->snString, num + 1,
...
int device_fpgas_pkt_rw(struct device *device)
{
	...
	for (num = 0; num < device->num_of_fpgas; num++) {
		...
		if (fpga->wr.io_state.pkt_comm_status) {
			fprintf(stderr, "SN %s FPGA #%d error: pkt_comm_status=0x%02x\n",
				device->ulx3s_device->snString, num + 1, fpga->wr.io_state.pkt_comm_status);
			return -1;
		}

		if (fpga->wr.io_state.app_status) {
			fprintf(stderr, "SN %s FPGA #%d error: app_status=0x%02x\n",
				device->ulx3s_device->snString, num + 1, fpga->wr.io_state.app_status);
			return -1;
		}
		...
		if (result < 0) {
			fprintf(stderr, "SN %s FPGA #%d write error: %d (%s)\n",
				device->ulx3s_device->snString, num + 1, result, libusb_error_name(result));
