/*
 * Functions for operating ULX3S 1.15y Multi-FPGA board at High-Speed
 * Require basic ULX3S functions (ulx3s.c)
...
struct device {
	struct ulx3s_device *ulx3s_device;
	struct device *next;
	int valid;
...
struct device_list {
	struct device *device;
	struct ulx3s_dev_list *ulx3s_dev_list;
};
...
// Underlying ulx3s_dev_list's also merged.
