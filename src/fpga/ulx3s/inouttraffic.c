...
// Creates 'struct device' on top of 'struct ulx3s_device'
struct device *device_new(struct ulx3s_device *ulx3s_device)
{
...
// underlying ulx3s_device also invalidated
void device_invalidate(struct device *device)
{
	...
	ulx3s_device_invalidate(device->ulx3s_device);
}
...
struct device_list *device_list_new(struct ulx3s_dev_list *ulx3s_dev_list)
{
	struct device_list *device_list = malloc(sizeof(struct device_list));
	if (!device_list)
		return NULL;
	device_list->device = NULL;
	device_list->ulx3s_dev_list = ulx3s_dev_list;
	if (!ulx3s_dev_list)
		return device_list;

	struct ulx3s_device *ulx3s_dev;
	struct device *device;
	for (ulx3s_dev = ulx3s_dev_list->dev; ulx3s_dev; ulx3s_dev = ulx3s_dev->next) {
		if (!ulx3s_device_valid(ulx3s_dev))
			continue;
		device = device_new(ulx3s_dev);
		if (!device)
			continue;
...
int device_list_merge(struct device_list *device_list, struct device_list *added_list)
{
	...
	for (dev = added_list->device; dev; dev = dev_next) {
		...
		if (DEBUG) printf("device_list_merge: SN %s, valid %d, next: %d\n",
				dev->ulx3s_device->snString, dev->valid, !!dev_next);
		if (!device_valid(dev)) {
			device_delete(dev);
			continue;
		}
	...
	int ulx3s_dev_count = ulx3s_dev_list_merge(device_list->ulx3s_dev_list,
			added_list->ulx3s_dev_list);
	if (ulx3s_dev_count != count) {
		printf("device_list_merge: ulx3s count %d, device count %d\n",
				ulx3s_dev_count, count);
	}
...
int device_check_bitstream_type(struct device *device, unsigned short bitstream_type)
{
	...
	for (i = 0; i < device->num_of_fpgas; i++) {
		int result;
		result = ulx3s_select_fpga(device->ulx3s_device, i);//fpga_select(&device->fpga[i]);
		if (result < 0)
			return result;
	...
	for (device = device_list->device; device; device = device->next) {
		...
		if (result > 0) {
			...
			printf("SN %s: device_list_check_bitstreams() failed: %d\n",
				device->ulx3s_device->snString, result);
			device_invalidate(device);
			continue;
		}
		...
		printf("SN %s: uploading bitstreams.. ", device->ulx3s_device->snString);
		fflush(stdout);

		result = ulx3s_upload_bitstream(device->ulx3s_device, fp);
		if (result < 0) {
			printf("failed\n");
			device_invalidate(device);
		}
